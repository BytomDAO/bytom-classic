...
func (Implementation) Dtrsm(s blas.Side, ul blas.Uplo, tA blas.Transpose, d blas.Diag, m, n int, alpha float64, a []float64, lda int, b []float64, ldb int) {
			...
			if ul == blas.Upper {
				for i := m - 1; i >= 0; i-- {
					...
					for ka, va := range a[i*lda+i+1 : i*lda+m] {
						k := ka + i + 1
						if va != 0 {
							f64.AxpyUnitaryTo(mityp, -va, b[k*ldb:k*ldb+n], mityp)
						}
					}
					if nonUnit {
						tmp := 1 / a[i*lda+i]
		...
		if ul == blas.Upper {
			for k := 0; k < m; k++ {
				...
				for ia, va := range a[k*lda+k+1 : k*lda+m] {
					...
					if va != 0 {
						mityp := b[i*ldb : i*ldb+n]
						f64.AxpyUnitaryTo(mityp, -va, mitypk, mityp)
					}
				}
				if alpha != 1 {
					for j := 0; j < n; j++ {
				...
				for k, vb := range mityp {
					if vb != 0 {
						if mityp[k] != 0 {
							...
							mitypk := mityp[k+1 : n]
							f64.AxpyUnitaryTo(mitypk, -mityp[k], a[k*lda+k+1:k*lda+n], mitypk)
						}
					}
				}
			}
		...
		for i := 0; i < m; i++ {
			...
			for k := n - 1; k >= 0; k-- {
				if mityp[k] != 0 {
					...
					f64.AxpyUnitaryTo(mityp, -mityp[k], a[k*lda:k*lda+k], mityp)
				}
			}
		}
		return
	...
	if ul == blas.Upper {
		for i := 0; i < m; i++ {
			mityp := b[i*lda : i*lda+n]
			for j := n - 1; j >= 0; j-- {
				tmp := alpha*mityp[j] - f64.DotUnitary(a[j*lda+j+1:j*lda+n], mityp[j+1:])
				if nonUnit {
					tmp /= a[j*lda+j]
				}
				mityp[j] = tmp
	...
	for i := 0; i < m; i++ {
		mityp := b[i*lda : i*lda+n]
		for j := 0; j < n; j++ {
			tmp := alpha*mityp[j] - f64.DotUnitary(a[j*lda:j*lda+j], mityp)
			if nonUnit {
				tmp /= a[j*lda+j]
			}
			mityp[j] = tmp
	...
	if ul == blas.Upper {
		for i := 0; i < n; i++ {
			...
			for l := 0; l < k; l++ {
				...
				mityp := b[l*ldb+i : l*ldb+n]
				if tmp1 != 0 || tmp2 != 0 {
					for j, v := range a[l*lda+i : l*lda+n] {
						ctmp[j] += v*tmp1 + mityp[j]*tmp2
					}
				}
			}
		}
...
func (Implementation) Dtrmm(s blas.Side, ul blas.Uplo, tA blas.Transpose, d blas.Diag, m, n int, alpha float64, a []float64, lda int, b []float64, ldb int) {
			...
			if ul == blas.Upper {
				for i := 0; i < m; i++ {
					...
					for ka, va := range a[i*lda+i+1 : i*lda+m] {
						...
						tmp := alpha * va
						if tmp != 0 {
							f64.AxpyUnitaryTo(mityp, tmp, b[k*ldb:k*ldb+n], mityp)
						}
					}
				}
				return
		...
		if ul == blas.Upper {
			for k := m - 1; k >= 0; k-- {
				...
				for ia, va := range a[k*lda+k+1 : k*lda+m] {
					...
					tmp := alpha * va
					if tmp != 0 {
						f64.AxpyUnitaryTo(mityp, tmp, mitypk, mityp)
					}
				}
				tmp := alpha
				if nonUnit {
		...
		for i := 0; i < m; i++ {
			...
			for k := 0; k < n; k++ {
				...
				if tmp != 0 {
					...
					f64.AxpyUnitaryTo(mityp, tmp, a[k*lda:k*lda+k], mityp)
				}
			}
		}
		return
	...
	if ul == blas.Upper {
		for i := 0; i < m; i++ {
			mityp := b[i*ldb : i*ldb+n]
			for j, vb := range mityp {
				tmp := vb
				if nonUnit {
					tmp *= a[j*lda+j]
				}
				tmp += f64.DotUnitary(a[j*lda+j+1:j*lda+n], mityp[j+1:n])
				mityp[j] = alpha * tmp
			}
		}
		return
	...
	for i := 0; i < m; i++ {
		...
		for j := n - 1; j >= 0; j-- {
			...
			tmp += f64.DotUnitary(a[j*lda:j*lda+j], mityp[:j])
			mityp[j] = alpha * tmp
		}
	}
}
